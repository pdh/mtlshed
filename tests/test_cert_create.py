...
@pytest.mark.parametrize("command,client_names,expected_files", [
    ("create", ["client1"], ["ca.crt", "server.key", "server.crt", "client1.pfx"]),
    ("create", ["client1", "client2"], ["ca.crt", "server.key", "server.crt", "client1.pfx", "client2.pfx"]),
])
def test_main_create(temp_dir, word_list_file, command, client_names, expected_files, monkeypatch):
    class MockArgs:
        def __init__(self):
